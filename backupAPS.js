      ...
      switch(action) {
        ...
        case 'installTrigger':
          return ContentService.createTextOutput(JSON.stringify({
            success: true,
            message: installProductionSummaryTrigger()
          })).setMimeType(ContentService.MimeType.JSON);
        default:
...
function getDashboardVersion() {
  try {
    ...
    if (!dataInvalidated) {
      try {
        ...
        if (cachedValue) {
          ...
          // Use cache if it's less than 30 seconds old (extended for event-driven)
    ...
    // Lightweight version check optimized for event-driven system
    const lastRow = sheet.getLastRow();
    
    // Simple hash - since we have event triggers, we can be even more minimal
    let quickHash = lastRow * 23; // Base on row count
    if (lastRow > 1) {
      try {
        // Only check 2 rows, 2 columns for ultra-minimal load
        const miniRange = sheet.getRange(2, 6, Math.min(2, lastRow - 1), 2);
        const values = miniRange.getValues();
        
        // Ultra-simple hash for event-driven system
        for (let i = 0; i < values.length; i++) {
          for (let j = 0; j < values[i].length; j++) {
            const val = String(values[i][j] || '');
            quickHash += val.length * (i + 1);
          }
        }
      } catch (hashError) {
        // Fallback to timestamp-based hash
        quickHash = lastRow * Math.floor(now / 60000); // Change every minute max
      }
    }
    ...
    // Cache for 30 seconds (extended since we have event invalidation)
    try {
      const cache = PropertiesService.getScriptProperties();
 ...
 */
function onProductionSummaryEdit(e) {
  try {
    console.log('📝 Edit trigger fired:', e);
    
    // Check if event object has the expected structure
    if (!e || !e.range) {
      console.log('⚠️ Edit event missing range data - invalidating cache anyway');
      // Even without range data, invalidate cache for any edit
      invalidateDashboardCache();
      return;
    }
    
    // Check if we can get the sheet from the event
    let sheet;
    if (e.source && e.source.getActiveSheet) {
      sheet = e.source.getActiveSheet();
    } else {
      // Fallback: try to get the sheet directly
      console.log('⚠️ Event missing source - using fallback sheet access');
      const ss = SpreadsheetApp.openById('1-WD-HFlRKFUJJvM1mwPKR07J_xZ2nwPaEobLlRoUlVc');
      sheet = ss.getSheetByName('Production Summary');
    }
    
    // Only process if this is the Production Summary sheet
    if (sheet && sheet.getName() !== 'Production Summary') {
      console.log(`ℹ️ Edit was on ${sheet.getName()} sheet - ignoring`);
      return; // Ignore edits to other sheets
    }
    ...
    // Only trigger for data rows (not header) and relevant columns
    // Columns 5-12 typically contain Status, Machine, Process Status, etc.
    if (editedRow > 1 && editedColumn >= 5 && editedColumn <= 12) {
      console.log(`📝 Production Summary edited - Row: ${editedRow}, Column: ${editedColumn}`);
      
      // Invalidate dashboard cache to force fresh data on next request
      invalidateDashboardCache();
      
      console.log('✅ Dashboard cache invalidated due to relevant edit');
      
    } else {
      console.log(`ℹ️ Edit outside relevant area - Row: ${editedRow}, Column: ${editedColumn} - not invalidating cache`);
    }
    
  } catch (error) {
    console.error('❌ onProductionSummaryEdit Error:', error);
    console.log('🔄 Invalidating cache anyway due to error');
    
    // Even if there's an error, invalidate cache to be safe
    try {
      invalidateDashboardCache();
    } catch (cacheError) {
      console.error('❌ Cache invalidation also failed:', cacheError);
    }
  }
}
...
function invalidateDashboardCache() {
  try {
    ...
    cache.deleteProperty('dashboardVersionCache');
    
    // Set invalidation flag with timestamp
    cache.setProperty('dashboardInvalidated', JSON.stringify({
      timestamp: new Date().getTime(),
      reason: 'Production Summary edited'
    }));
    
    console.log('🗑️ Dashboard cache invalidated due to data change');
  } catch (error) {
 ...
 * DASHBOARD-ONLY FUNCTION
 */
function checkForProductionSummaryChanges() {
  try {
    ...
    if (!sheet) {
      console.log('⚠️ Production Summary sheet not found');
      return;
    }
    
    // Get the last modified time or a simple checksum
    const lastRow = sheet.getLastRow();
    const lastColumn = sheet.getLastColumn();
    
    if (lastRow > 0 && lastColumn > 0) {
      // Create a simple checksum of the sheet dimensions and a sample of data
      const range = sheet.getRange(1, 1, Math.min(lastRow, 50), lastColumn);
      const values = range.getValues();
      const checksum = JSON.stringify(values).length + '_' + lastRow + '_' + lastColumn;
      
      const lastChecksum = properties.getProperty('production_summary_checksum');
      
      if (lastChecksum !== checksum) {
        console.log('📊 Production Summary changes detected - invalidating cache');
        
        // Update checksum
        properties.setProperty('production_summary_checksum', checksum);
        
        // Invalidate cache (same logic as edit trigger)
        invalidateDashboardCache();
        
        console.log(`✅ Changes processed, checksum updated`);
      } else {
        console.log('✅ No changes detected in Production Summary');
      }
    }
    
  } catch (error) {
    console.error('❌ Error checking for Production Summary changes:', error);
  }
}

/**
 * Install the Production Summary edit trigger for INSTANT updates
 * Run this ONCE to set up real-time event-driven updates
 */
function installProductionSummaryTrigger() {
  try {
    // Delete existing triggers if any
    const triggers = ScriptApp.getProjectTriggers();
    triggers.forEach(trigger => {
      if (trigger.getHandlerFunction() === 'onProductionSummaryEdit' || 
          trigger.getHandlerFunction() === 'checkForProductionSummaryChanges') {
        ScriptApp.deleteTrigger(trigger);
      }
    });
    
    // Get the Production Summary spreadsheet
    const ss = SpreadsheetApp.openById('1-WD-HFlRKFUJJvM1mwPKR07J_xZ2nwPaEobLlRoUlVc');
    
    // CORRECT WAY: Edit triggers are created using onFormSubmit() for forms or
    // For spreadsheets, we need to use the spreadsheet-specific trigger creation
    // Unfortunately, Google Apps Script doesn't allow programmatic creation of onEdit triggers
    // They must be created manually through the UI
    
    console.log('❌ Edit triggers cannot be created programmatically');
    console.log('📝 Edit triggers must be set up manually through the Google Apps Script UI');
    
    // Provide ONLY manual edit trigger instructions
    const manualInstructions = `
🚀 MANUAL EDIT TRIGGER SETUP (for INSTANT updates):

1. Open Google Apps Script Editor (script.google.com)
2. Open your project with this code
3. Click "Triggers" (clock icon) in left sidebar
4. Click "+ Add Trigger"
5. Choose function: onProductionSummaryEdit
6. Choose event source: From spreadsheet
7. Choose spreadsheet: (select your Production Summary spreadsheet)
8. Choose event type: On edit
9. Click "Save"

✅ Result: Your dashboard will update INSTANTLY when data changes!

Note: The onProductionSummaryEdit function is already coded and ready.
This is a Google Apps Script limitation - edit triggers must be set up manually.
`;
    
    console.log('📋 Manual setup instructions:', manualInstructions);
    return manualInstructions;
    
  } catch (error) {
    console.error('❌ Error in trigger setup:', error);
    return 'Error: Edit triggers must be set up manually through Google Apps Script UI';
  }
}

/**
 * Delete the time-based trigger (cleanup function)
 * Run this to remove the fallback time-based trigger
 */
function deleteTimeTrigger() {
  try {
    ...
    triggers.forEach(trigger => {
      if (trigger.getHandlerFunction() === 'checkForProductionSummaryChanges') {
        ScriptApp.deleteTrigger(trigger);
        deletedCount++;
        console.log('🗑️ Deleted time-based trigger:', trigger.getUniqueId());
      }
    });
    
    if (deletedCount > 0) {
      console.log(`✅ Successfully deleted ${deletedCount} time-based trigger(s)`);
      console.log('🎯 Now ready for edit trigger setup only!');
      return `Deleted ${deletedCount} time-based trigger(s). Ready for edit trigger setup!`;
    } else {
      console.log('ℹ️ No time-based triggers found to delete');
      return 'No time-based triggers found to delete';
    }
    
  } catch (error) {
    console.error('❌ Error deleting time trigger:', error);
    return 'Error deleting time trigger: ' + error.message;
  }
}